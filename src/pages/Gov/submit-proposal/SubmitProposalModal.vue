      ...
      <div class="space-y-6">
        ...
        <section class="space-y-2">
          ...
          <div class="flex flex-wrap gap-2">
            <UButton
              v-for="a in actionTabs"
              :key="a.id"
              color="neutral"
              :variant="selectedAction === a.id ? 'solid' : 'outline'"
              size="lg"
...
const canSubmit = computed(() => {
  ...
  return s.to.trim().length > 0 && s.amount.trim().length > 0
})
...
function reset() {
  selectedAction.value = (props.defaultAction === 'addTrack' || props.defaultAction === 'spend' ? props.defaultAction : '') as ActionId | ''
