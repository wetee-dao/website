 ...
 */
export const Metamask: InjectedWindowProvider = {
  connect,
  enable: connect,
  version: DEFAULT_SNAP_VERSION
};
...
// 设置自定义链
export const setCustomChain = async (wsRpcUrl: string, metaData: MetadataDef) => {
  await isPolkaMaskInstalled()
  wsRpcUrl = wsRpcUrl.replace('wss://', 'https://')
  wsRpcUrl = wsRpcUrl.replace('ws://', 'http://')
  return await invokeSnap({
    method: 'configure',
    params: {
      configuration: {
        networkName: metaData.chain,
        wsRpcUrl: wsRpcUrl,
        addressPrefix: metaData.ss58Format,
        unit: {
          image: "",
